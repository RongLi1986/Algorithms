    ...
    vector<vector<int>> consistentHashing(int n) {
        ...
        for(int i=2;i<=n;i++){
            ...
            for(int j=0;j<res.size();j++){
                int delta = res[j][1]-res[j][0]+1;
                if(delta>maxLength ){
                    maxLength = delta;
                    maxIndex = res[j][2];
                }else if(delta==maxLength ){
                    if(res[j][2]<maxIndex){
                        maxLength = delta;
                        maxIndex = res[j][2];
                    }
            ...
            for(int j=0;j<size;j++){
                if(res[j][2]==maxIndex){
                    vector<int> tmp;
                    if(maxLength%2==0){  //???
                        res[j][1]=res[j][0]+maxLength/2-1;
                    }else{
                        res[j][1]=res[j][0]+maxLength/2;
                    }
                    tmp.push_back(res[j][1]+1);
                    tmp.push_back(res[j][1]+maxLength/2);
                    tmp.push_back(i);
